 ...
 */
export const useFluidcursor = (key: string) => {
  ...
  const _blurVertexShader = compileShader(
    gl.VERTEX_SHADER,
    `
       precision highp float;
  ...
  const _blurShader = compileShader(
    gl.FRAGMENT_SHADER,
    `
  ...
  const _colorShader = compileShader(
    gl.FRAGMENT_SHADER,
    `
       precision mediump float;
  ...
  function _createTextureAsync(url) {
    const texture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, texture);
